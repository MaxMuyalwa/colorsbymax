      ...
      {open && (
        <div className="fixed inset-0 z-[80] flex items-end justify-center sm:items-center sm:p-6">
          ...
          <div
            ...
            className="relative w-full max-w-md animate-[tab-in_300ms_ease-out] overflow-hidden rounded-t-[2rem] border border-border bg-surface text-left shadow-2xl shadow-shadow/30 sm:rounded-[2rem]"
          >
            <div className="relative overflow-hidden border-b border-border px-6 pt-6 pb-5">
              <div className="blob -top-20 -right-10 h-44 w-44 bg-primary-alt opacity-25" aria-hidden="true" />
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <ZambiaFlag className="h-8 w-12 rounded shadow-md ring-1 ring-border" />
                  <h2 id={`${id}-title`} className="mt-4 font-display text-2xl font-bold text-ink">
                    Support from Zambia or Africa
                  </h2>
                  <p className="mt-1 text-sm text-ink-secondary">If you live in Zambia or elsewhere in Africa, you can say thanks with Airtel Money. Any amount you like.</p>
                </div>
                <button ref={closeRef} type="button" onClick={close} aria-label="Close" className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full text-ink transition hover:bg-accent hover:text-on-accent">
            ...
            <div className="space-y-5 px-6 py-6">
              <div className="rounded-2xl border border-border bg-background p-4">
                ...
                <div className="mt-1 flex flex-wrap items-center justify-between gap-3">
                  <p className="font-mono text-2xl font-bold tracking-wide text-ink">{NUMBER}</p>
                  <button
                    type="button"
                    onClick={copy}
                    className="inline-flex cursor-pointer items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-on-primary shadow-md shadow-primary/25 transition hover:-translate-y-0.5"
                  >
              ...
              <ol className="space-y-2 text-sm text-ink-secondary">
                {['Open Airtel Money on your phone.', 'Choose Send money and enter the number above.', `Check the name shows ${NAME}, then send whatever you’d like.`].map((step, i) => (
              ...
              <p className="flex items-center gap-2 rounded-2xl bg-secondary p-4 text-sm font-semibold text-on-secondary">
                <Heart className="h-5 w-5 shrink-0" aria-hidden="true" /> Thank you in advance. Every bit of support keeps colorsbymax going.
              </p>
            </div>
          </div>
        </div>
